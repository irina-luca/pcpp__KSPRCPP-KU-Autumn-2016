...
class Example154 {
  public static void main(String[] args) {
    FunList<Integer> empty = new FunList<>(null),
      ...
      // // Exercise 3.1.1 : TEST
      // System.out.println("Exercise 3.1.1 : TEST");
      // System.out.println(list7.remove(13));
      // System.out.println(list7.remove(7).remove(13));
      // // Exercise 3.1.2 : TEST
      // System.out.println("Exercise 3.1.2 : TEST");
      // System.out.println(list7.count(x -> x%2 == 0));
      // System.out.println(list7.count(x -> x%2 != 0));
      // // Exercise 3.1.3 : TEST
      // System.out.println("Exercise 3.1.3 : TEST");
      // System.out.println(list7.filter(x -> x%7 == 0));
      // System.out.println(list7.filter(x -> x%7 != 0));
      // Exercise 3.1.4 : TEST
      System.out.println("Exercise 3.1.4 : TEST");
      System.out.println(list7.removeFun(13));
      System.out.println(list7.removeFun(7).removeFun(13));
      // // Exercise 3.1.5 : TEST
      // System.out.println("Exercise 3.1.5 : TEST");
      // FunList<FunList<Integer>> finalList = new FunList<FunList<Integer>>(null);
      // finalList = cons(list1, cons(list2, cons(list4, finalList)));
      // // FunList.flatten(finalList).forEach(y -> System.out.println(y));
      // // finalList.forEach(x -> x.forEach(y -> System.out.println(y)));
      // // Exercise 3.1.6 : TEST
      // System.out.println("Exercise 3.1.6 : TEST");
      // // FunList.flattenFun(finalList).forEach(y -> System.out.println(y));
      // // Exercise 3.1.7 : TEST
      // System.out.println("Exercise 3.1.7 : TEST");
      // System.out.println(list7.flatMap(y -> cons(y+1, empty)));
      // System.out.println(list7.flatMapFun(y -> cons(y+1, empty)));
      // // Exercise 3.1.8 : TEST
      // System.out.println("Exercise 3.1.8 : TEST");
      // System.out.println(list4.scan((x, y) -> x*y));
...
class FunList<T> {
  ...
  public FunList<T> remove(T item) {
    return new FunList<T>(remove(item, this.first));
  }

  protected static <T> Node<T> remove(T item, Node<T> xs) {
    return (xs != null) ? ((xs.item == item) ? remove(item, xs.next) : new Node<T>(xs.item, remove(item, xs.next)) ) : getNodeRecursive(0, xs);
  }
  ...
  protected static <T> int count(Predicate<T> p, Node<T> xs) {
    if(xs == null) return 0;
    return p.test(xs.item) ? 1 + count(p, xs.next) : count(p, xs.next);
  }
  // Exercise 3.1.2: Naive version
  // public int count(Predicate<T> p) {
  //   Node<T> xs = first;
  //   int count = 0;
  //   while (xs != null) {
  ...
  protected static <T> Node<T> filter(Predicate<T> p, Node<T> xs) {
    return (xs != null) ? ((!p.test(xs.item)) ? filter(p, xs.next) : new Node<T>(xs.item, filter(p, xs.next)) ) : getNodeRecursive(0, xs);
  }

  // Exercise 3.1.4
  public FunList<T> removeFun(T item) {
    // return new FunList<T>(filter((T x) -> (x != item), this.first));
    return new FunList<T>(filter((T x) -> (!x.equals(item)), this.first));

  }

  // Exercise 3.1.5
  public static <T> FunList<T> flatten(FunList<FunList<T>> xss) {
    FunList<T> result = new FunList<T>(null);
    return (xss.getCount() != 0) ? xss.get(0).append(flatten(xss.removeAt(0))) : result;
  }

  // Exercise 3.1.6
  public static <T> FunList<T> flattenFun(FunList<FunList<T>> xss) {
    return xss.reduce(new FunList<T>(null), (x, y) -> x.append(y));
  }
  ...
  private <U> FunList<U> flatMap(Function<T,FunList<U>> f, Node<T> xs) {
    return xs == null ? new FunList<U>(null) : f.apply(xs.item).append(flatMap(f, xs.next));
  }
  ...
  public FunList<T> scan(BinaryOperator<T> f) {
    return scan(f, this.first);
  }

  protected static <T> FunList<T> scan(BinaryOperator<T> f, Node<T> xs) {
    Node<T> result = new Node<T>(xs.item, null);    
    while(xs != null && xs.next != null) {
      result = new Node<T>(f.apply(xs.next.item, xs.item), result);
      xs = xs.next;      
    }
    FunList<T> finalList = new FunList<T>(result);
