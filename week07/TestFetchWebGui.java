...
public class TestFetchWebGui {
  public static void main(String[] args) {
    // badFetch(); // it shows stuff gradually because it runs on the main thread
    // goodFetch();
    goodFetchOwn();
  }
  ...
  private static void goodFetchOwn() {
    final JFrame frame = new JFrame("TestFetchWebGui");
    final JPanel outerPanel = new JPanel(), 
      buttonPanel = new JPanel();
    final JButton fetchButton = new JButton("Fetch"), 
    ...
    outerPanel.add(textArea, BorderLayout.WEST);
    // (1) Use a background thread, not the event thread, for work
    // Use N DownloadWorkers
    DownloadWorkerOwn[] downloadWorkersOwn = new DownloadWorkerOwn[urls.length];

    for(int i = 0; i < urls.length; i++) { // N = urls.length
      DownloadWorkerOwn downloadTask = new DownloadWorkerOwn(textArea, urls[i]);
      downloadWorkersOwn[i] = downloadTask;
      fetchButton.addActionListener(new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            for(int i = 0; i < urls.length; i++) {
              downloadWorkersOwn[i].execute();
             }
          }});
    ...
    AtomicInteger aiProgress = new AtomicInteger(0);

    for(int i = 0; i < urls.length; i++) {
     downloadWorkersOwn[i].addPropertyChangeListener(new PropertyChangeListener() {
        public void propertyChange(PropertyChangeEvent e) { // when call publish down there
          int val = aiProgress.addAndGet(1);

          // if ("progress".equals(e.getPropertyName())) { // if it comes from progress only!!!
    ...
    cancelButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
           for(int i = 0; i < urls.length; i++) {
            downloadWorkersOwn[i].cancel(false);
           }
        }});
    frame.pack(); frame.setVisible(true);
  }
  ...
  static class DownloadWorkerOwn extends SwingWorker<String,String> {
    private final TextArea textArea; 
    private final String url;

    public DownloadWorkerOwn(TextArea textArea, String url) {
      this.textArea = textArea;
      this.url = url;
    }
    ...
    public String doInBackground() {
      ...
      // for (String url : urls) {
        if (!isCancelled()) {
  	      System.out.println("Fetching " + url);
          String page = getPage(url, 200),
          res = String.format("%-40s%7d%n", url, page.length());
          result = res;
        }      // (3)
        //   break;
        // sb.append(result); // (1)
