...
class StmMap<K,V> implements OurMap<K,V> {
  ...
  // Exercise 9.3.1: (b) use atomic only around the code that accesses the buckets array reference and indexes into the array
  ...
  public V get(K k) {
    ...
    return atomic(() ->  ItemNode.search(bs[hash].get(), k, old)) ? old.get() : null; // NOT sure about this: IRINA   sizes.get(stripe) != 0 && 
  }
  ...
  public V put(K k, V v) {
    ...
    // int afterSize;
    final ItemNode<K,V> node = bs[hash].atomicGet(), 
                        newNode = ItemNode.delete(node, k, old);
    atomic(() -> {
      bs[hash] = StmUtils.<ItemNode<K,V>>newTxnRef(new ItemNode<K,V>(k, v, newNode));
    });
    // Write for visibility; increment if k was not already in map
    if(newNode == node) 
      cachedSize.atomicGetAndIncrement(1);
    else
      cachedSize.atomicGetAndIncrement(0);
  ...
  public V putIfAbsent(K k, V v) {
    ...
    // int afterSize;
    atomic(() -> {
      final ItemNode<K,V> bl = bs[hash].atomicGet(); 
      boolean nodeFound = ItemNode.search(bl, k, old);
      if(!nodeFound) { // if k is not already in the hash map
        bs[hash] = StmUtils.<ItemNode<K,V>>newTxnRef(new ItemNode<K,V>(k, v, bl));
        cachedSize.increment(1);
        // afterSize = sizes.addAndGet(stripe, 1);
      } else {
        cachedSize.increment(0);
  ...
  // Exercise 9.3.2: the call to consumer(node.k, node.v) should clearly not be inside a transaction
